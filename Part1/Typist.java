...
public class Typist
{
   ...
   public void burnOut(int turns)
    ...
    {
        if (this.isBurntOut) {
            this.burnoutTurnsRemaining -= 1; // reduce burnout turns by one
            if (this.burnoutTurnsRemaining <= 0) {
                this.isBurntOut = false; // recover from burnout when counter reaches zero
                this.burnoutTurnsRemaining = 0; // reset burnout turns to zero
            }
        }
    }
   ...
   public void resetToStart()
    ...
    /**
     * Returns the number of times this typist has been burnt out.
     *
     * @return the burnout count as a non-negative integer
     */
    ...
    {
        if (!isBurntOut) {
            progress += 1; // advance progress by one character
        }
    }
